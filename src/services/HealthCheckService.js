    ...
    this.registerCheck('sessions', async () => {
      ...
      for (const session of sessions.values()) {
        switch (session.status) {
          ...
          case 'authenticated': {
            sessionStats.authenticated++;
            break;
          }
          case 'disconnected': {
            sessionStats.disconnected++;
            break;
          }
          case 'error': {
            {
      ...
      const healthyRatio =
        (sessionStats.ready + sessionStats.authenticated) / Math.max(1, sessionStats.total);
          ...
          switch (check) {
            ...
            case 'system': {
              recommendations.push(
                'System resources are constrained, consider scaling or optimization',
              );
              break;
            }
            case 'sessions': {
