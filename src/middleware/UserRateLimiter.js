...
class UserRateLimiter {
  ...
  async checkLimit(userId, weight = 1) {
    ...
    // Clean old requests
    userWindow.requests = userWindow.requests.filter((req) => now - req.timestamp < this.windowMs);

    // Check burst limit
    const recentBurst = userWindow.requests
      .filter((req) => now - req.timestamp < 1000)
  ...
  getUserStats(userId) {
    ...
    const now = Date.now();
    const activeRequests = userWindow.requests.filter((req) => now - req.timestamp < this.windowMs);
    ...
    return {
      ...
      newestRequest: activeRequests.at(-1)?.timestamp,
    };
  }
