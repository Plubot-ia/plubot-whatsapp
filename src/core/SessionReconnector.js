...
export class SessionReconnector extends EventEmitter {
  constructor(config = {}) {
    ...
    this.config = {
      ...
      reconnectTimeout: config.reconnectTimeout || 300000, // 5 minutes
      ...config,
    };
  ...
  async _attemptReconnection(sessionId) {
    ...
    logger.info(`Scheduling reconnection for session ${sessionId} in ${delay}ms (attempt ${attempts + 1}/${this.config.maxRetries})`);
